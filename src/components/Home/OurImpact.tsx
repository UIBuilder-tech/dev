...
export default function ImpactSection() {
  ...
  return (
    <div className="py-8 px-4 md:p-8 bg-cream md:mx-14">
      <h2 className="text-3xl md:text-4xl font-display text-gray-900 mb-8">Our Impact</h2>
              ...
              <div className="absolute inset-0 p-4 md:p-8 flex flex-col justify-between">
                <div className="flex justify-between items-start w-full">
                  <div className="space-y-2">
                    <div>
                    <p className="text-white text-xl md:text-2xl font-medium">{activeCard.title}</p>
                    {activeCard.subtitle && (
                      <p className="text-white/90 text-sm md:text-xl">{activeCard.subtitle}</p>
                    )}
                    </div>
                ...
                <div className={`h-2 z-[10] rounded-xl t-2 bg-[#f97316] relative top-[5px] shadow-md transition-all duration-500 w-[12rem] md:w-[50rem]`} />
                  <div 
                    className="bg-white rounded-[20px] p-4 md:p-6 w-full  relative min-h-[150px]"
                  >
                    <p className="max-sm:text-xs text-sm text-[#516072] max-sm:leading-5 ">{activeCard.description}</p>
            ...
            <div className="absolute inset-0 p-2 md:p-6 flex flex-col justify-between">
              <div className="flex md:flex-row flex-col h-full justify-between items-end">
                <div className="space-y-2 text-start w-full">
                    <div>
                  <p className="text-white text-md md:text-xl font-medium">{card.title}</p>
                  {card.subtitle && (
                    <p className="text-white/90 text-xs md:text-md">{card.subtitle}</p>
                  )}
                  </div>
