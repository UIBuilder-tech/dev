...
export default function GrantsSection() {
  ...
  useEffect(() => {
    const api = async () => {
      ...
      fetch(`${AdminPanelUrl}/project-page?populate[grant_lists][populate]=*`, requestOptions)
        .then(response => response.json())
        .then(result => {
          if (result?.data) {
            if (result?.data?.grant_lists) {
              setPageData(v => ({ ...v, grant_lists: result.data.grant_lists }))
            }
  ...
  return (
    <>
      {PageData && <div className="max-w-7xl desktop-1900:max-w-[80%] mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 ">
              ...
              PageData?.grant_lists && PageData.grant_lists.map((item, index) => (
                <div key={`grant_${index}`}>
                  ...
                  <div className="space-y-6 py-4">
                    {
                      item?.list && item.list.map((listItem: GrantListItem, i: number) => (
