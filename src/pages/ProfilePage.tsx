...
export default function Profile() {
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/api/contact?email=${user?.email}`).then(resp=>resp?.json())
              .then(response => {
                if (response) {
                  setPersonalDetails({
    ...
    <div className="max-w-4xl mx-auto p-6 pt-48 space-y-8 max-sm:space-y-6 max-sm:py-[80px]">
      ...
      <div className="bg-white rounded-lg p-8 max-sm:p-6 shadow-lg border border-gray-200">
        <div className="flex justify-between md:items-center mb-8 max-sm:flex-col">
          <p className="text-2xl md:text-3xl font-semibold text-gray-700">Personal Details</p>
          ...
          {isEditingPersonal ? (
            ...
            <>
              <div className="space-y-3">
                ...
                <p className="text-xl max-sm:text-lg text-gray-800">{personalDetails.FirstName}</p>
              </div>
              <div className="space-y-3">
                <label className="block text-sm text-gray-500">Last Name</label>
                <p className="text-xl max-sm:text-lg text-gray-800">{personalDetails.LastName}</p>
              </div>
              <div className="space-y-3">
                <label className="block text-sm text-gray-500">Email Address</label>
                <p className="text-xl max-sm:text-lg text-gray-800">{personalDetails.Email}</p>
              </div>
              <div className="space-y-3">
                <label className="block text-sm text-gray-500">Mobile Number</label>
                <p className="text-xl max-sm:text-lg text-gray-800">{personalDetails.Phone}</p>
              </div>
            </>
          )}
        </div>
      ...
      <div className="bg-white rounded-lg p-8 max-sm:p-6 shadow-lg border border-gray-200">
        <div className="flex justify-between items-center mb-8">
          <p className="text-2xl md:text-3xl font-semibold text-gray-700">Home Address</p>
          {!isEditingAddress && (
            <button
        ...
        {isEditingAddress ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="md:col-span-2">
              ...
              <p className="text-xl max-sm:text-lg text-gray-800">{address.street || "Not provided"}</p>
            </div>
            <div>
              <label className="block text-sm text-gray-500">City</label>
              <p className="text-xl max-sm:text-lg text-gray-800">{address.city || "Not provided"}</p>
            </div>
            <div>
              <label className="block text-sm text-gray-500">State</label>
              <p className="text-xl max-sm:text-lg text-gray-800">{address.state || "Not provided"}</p>
            </div>
            <div>
              <label className="block text-sm text-gray-500">Zip Code</label>
              <p className="text-xl max-sm:text-lg text-gray-800">{address.zipCode || "Not provided"}</p>
            </div>
            <div>
              <label className="block text-sm text-gray-500">Country</label>
              <p className="text-xl max-sm:text-lg text-gray-800">{address.country}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="fixed z-[9999] inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg p-8 max-w-2xl w-full">
          <div className="flex justify-between items-center mb-6">
            <p className="text-2xl md:text-3xl font-semibold text-gray-800">Family Member Details</p>
            <button onClick={() => setIsFamilyModalOpen(false)} className="text-gray-500 hover:text-gray-700">
    ...
    {isPasswordModalOpen && (
      <div className="fixed z-[9999] inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg p-8 max-w-md w-full">
          <div className="flex justify-between items-center mb-6">
            <p className="text-2xl md:text-3xl font-semibold text-gray-800">Change Password</p>
